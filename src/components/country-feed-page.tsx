...
export function CountryFeedPage({ initialFeed, initialFilter, searchTag }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      {/* En PC se pega bajo FinanceTopBar (h-12); en móvil sin top bar */}
      <header className="sticky top-0 md:top-12 z-30 w-full bg-background/90 backdrop-blur-xl border-b border-border/60">
        {/* pl-14: hueco para el círculo hamburguesa flotante en móvil */}
        <div className="max-w-[1400px] mx-auto pl-14 pr-2 sm:px-6 h-12 flex items-center gap-2">
          {/* Tabs: scroll horizontal en móvil para que no se aplasten */}
          <div className="flex-1 min-w-0 overflow-x-auto hide-scrollbar h-full">
            <div className="flex items-center gap-3 sm:gap-6 text-[13px] sm:text-sm font-semibold h-full relative w-max min-w-full sm:min-w-0 sm:justify-center px-1">
              <button
                onClick={() => {
                  triggerTransition();
                ...
                className={`h-full shrink-0 border-b-2 flex items-center px-1 transition-all relative ${
                  filterMode === 'para_ti'
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="h-full shrink-0 border-b-2 border-transparent text-muted-foreground hover:text-foreground flex items-center gap-1 transition-all outline-none">
      ...
      <div className="max-w-[1400px] mx-auto px-3 sm:px-6 pb-8 sm:pb-16 pt-3 sm:pt-4">
              ...
              <div className={`flex flex-col ${gapClass}`}>
                ...
                <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-4 sm:gap-6">
                    {/* Hero Article: imagen arriba en móvil, texto + imagen en desktop */}
                    {visibleArticles[0] && (
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-6 bg-card rounded-2xl border border-border overflow-hidden hover:shadow-lg hover:shadow-blue-500/5 transition-all group">
                        {visibleArticles[0].image_url && (
                          <div
                            className="md:col-span-5 order-1 md:order-2 aspect-[16/10] sm:aspect-[16/9] md:aspect-auto md:min-h-[240px] overflow-hidden cursor-pointer"
                        ...
                        <div className={`md:col-span-7 order-2 md:order-1 p-4 sm:p-6 md:p-8 flex flex-col justify-center gap-2.5 sm:gap-4 ${!visibleArticles[0].image_url ? 'md:col-span-12' : ''}`}>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-blue-500">{visibleArticles[0].category}</span>
                            <span className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600" />
                            <span className="text-[11px] sm:text-xs text-gray-500 font-medium">{formatTimeAgo(visibleArticles[0].published_at)}</span>
                          </div>
                          <h2
                            className="text-xl sm:text-2xl md:text-3xl font-bold leading-snug sm:leading-tight text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors cursor-pointer"
                            onClick={() => openArticle(visibleArticles[0].id, visibleArticles[0])}
                          >
                            {visibleArticles[0].title}
                          </h2>
                          <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed line-clamp-2 sm:line-clamp-3">
                            {visibleArticles[0].summary}
                          </p>
                          <div className="flex items-center gap-3 text-xs text-gray-400 font-medium mt-auto pt-1">
                    ...
                    {/* Row of cards: 1 col móvil, 3 en sm+ */}
                    ...
                    {/* Horizontal Articles — compactos en móvil */}
                    {visibleArticles.slice(4).map((article, i) => (
                      <article
                        key={article.id}
                        className="group flex gap-3 sm:gap-4 bg-card rounded-xl border border-border p-3 sm:p-4 hover:shadow-md hover:shadow-blue-500/5 transition-all cursor-pointer"
                        onClick={() => openArticle(article.id, article)}
                      >
                        {article.image_url && (
                          <div className="w-24 h-20 sm:w-40 sm:h-28 rounded-lg sm:rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 dark:bg-gray-800">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center gap-1 sm:gap-1.5">
                          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-blue-500 truncate">{article.category}</span>
                            <span className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600 shrink-0" />
                            <span className="text-[10px] sm:text-[11px] text-gray-400 font-medium truncate">{article.sources?.[0]?.name || formatTimeAgo(article.published_at)}</span>
                          </div>
                          <h3 className="text-sm sm:text-lg font-bold text-gray-900 dark:text-white line-clamp-2 group-hover:text-blue-500 transition-colors leading-snug">
                            {article.title}
                          </h3>
                          <p className="text-xs text-gray-400 line-clamp-1 hidden sm:block">{article.summary}</p>
                          <div className="hidden sm:flex items-center gap-2 text-[10px] text-gray-400 font-medium mt-1">
