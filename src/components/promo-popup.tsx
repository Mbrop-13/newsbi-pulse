...
export function PromoPopup() {
  ...
  useEffect(() => {
    ...
    const dismissedAt = localStorage.getItem("maverlang_promo_dismissed");
    if (dismissedAt) {
  ...
  const handleDismiss = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setIsVisible(false);
    localStorage.setItem("maverlang_promo_dismissed", Date.now().toString());
  };
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className="absolute -inset-[2px] rounded-[26px] bg-gradient-to-br from-zinc-700 via-neutral-900 to-black dark:from-white dark:via-zinc-500 dark:to-neutral-900 blur-xl opacity-40 group-hover:opacity-75 transition-opacity duration-1000 animate-pulse-slow"></div>
          ...
          <div className="relative overflow-hidden bg-white/95 dark:bg-[#070A11]/95 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-3xl shadow-2xl p-6 group">
            ...
            <motion.div 
              ...
              className="absolute -top-32 -right-32 w-64 h-64 bg-gradient-to-tr from-zinc-700 to-neutral-900 dark:from-white dark:to-zinc-500 blur-3xl opacity-20 dark:opacity-30 rounded-full mix-blend-screen"
            />
            <motion.div 
              animate={{ rotate: -360 }}
              transition={{ repeat: Infinity, duration: 25, ease: "linear" }}
              className="absolute -bottom-32 -left-32 w-64 h-64 bg-gradient-to-tr from-zinc-500 to-neutral-800 blur-3xl opacity-20 dark:opacity-30 rounded-full mix-blend-screen"
            />
            ...
            <div className="relative z-10 flex flex-col gap-5">
              <div className="flex items-start gap-4">
                <div className="relative p-3 rounded-2xl bg-gradient-to-br from-zinc-200 to-zinc-300 dark:from-zinc-800 dark:to-zinc-900 text-black dark:text-zinc-200 shadow-inner group-hover:scale-110 transition-transform duration-500">
                  ...
                  <motion.div animate={{ opacity: [0, 1, 0], scale: [0.8, 1, 0.8] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute -top-1 -right-1 text-zinc-500"><Sparkles className="w-3 h-3" /></motion.div>
                </div>
                
                <div className="pt-1">
                  <h3 className="text-[22px] font-black tracking-tight text-gray-900 dark:text-white leading-none mb-1.5">
                    Maverlang <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-700 to-black dark:from-white dark:to-zinc-400">Max</span>
                  </h3>
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-[11px] font-black text-black dark:text-zinc-400 uppercase tracking-widest">
              ...
              <div className="grid grid-cols-2 gap-3 mb-2 bg-gray-50/50 dark:bg-white/[0.03] p-3 rounded-2xl border border-gray-100 dark:border-white/5">
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <Cpu className="w-4 h-4 text-black dark:text-white" /> Modelos IA
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <TrendingUp className="w-4 h-4 text-zinc-500" /> Mercados PRO
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <Zap className="w-4 h-4 text-zinc-500" /> Velocidad Real
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                  <Sparkles className="w-4 h-4 text-zinc-500" /> Sin Límites
                </div>
              </div>
              ...
              <Link 
                ...
                className="group/btn relative w-full flex items-center justify-center gap-2 mt-1 px-5 py-3.5 rounded-2xl bg-black text-white dark:bg-white dark:text-black font-black text-[15px] transition-all hover:scale-[1.02] shadow-[0_8px_30px_rgba(0,0,0,0.15)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.25)] dark:shadow-white/5 overflow-hidden"
              >
                {/* Shine effect */}
