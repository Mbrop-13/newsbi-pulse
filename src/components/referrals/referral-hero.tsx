...
export function ReferralHero({ code }: { code: string }) {
  ...
  const channels = [
    {
      ...
      shadow: "shadow-zinc-950/20 dark:shadow-black/40",
      action: () =>
        window.open(
  ...
  return (
    ...
    >
      <div className="relative rounded-[32px] bg-gradient-to-br from-zinc-800 via-zinc-900 to-black p-[1px] shadow-2xl shadow-zinc-800/10 overflow-hidden">
        <div className="rounded-[31px] overflow-hidden relative bg-zinc-900 dark:bg-black">
          ...
          <div className="absolute -bottom-20 -left-10 w-56 h-56 bg-zinc-500/10 blur-3xl rounded-full pointer-events-none" />

          <div className="relative z-10 p-6 md:p-10 text-white">
            {/* Eyebrow */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-[11px] font-bold uppercase tracking-widest mb-4">
            ...
            <button
              onClick={copyLink}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 transition-all group text-left cursor-pointer"
            >
              ...
              <span className="text-[11px] font-bold text-white shrink-0 px-3 py-1.5 rounded-lg bg-white/10 group-hover:bg-white/20 transition-colors">
            ...
            <div className="grid grid-cols-4 gap-2 mt-3">
              {channels.map((ch) => (
                <button
                  key={ch.name}
                  onClick={ch.action}
                  className="flex flex-col items-center gap-1.5 p-2.5 rounded-2xl hover:bg-white/5 transition-all group cursor-pointer"
                >
                  <div
