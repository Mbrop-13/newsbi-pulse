...
export type SubscriptionContentVariant = "page" | "dialog";

export interface SubscriptionContentProps {
  /** page = ruta /suscripcion; dialog = overlay sobre la app */
  variant?: SubscriptionContentVariant;
  onClose?: () => void;
  /** ?status= de la URL (solo modo page / retorno de pago) */
...
export function SubscriptionContent({
  ...
  return (
    <div
      className={cn(
        "bg-[#f8f8fb] dark:bg-zinc-950 text-neutral-900 dark:text-neutral-100 transition-colors duration-300 relative animate-fade-in",
        isDialog ? "pb-10" : "min-h-screen pb-24"
      )}
    >
      {/* Close: en page va a home; en dialog cierra el overlay */}
      {isDialog ? (
        <button
          type="button"
          onClick={() => onClose?.()}
      ...
      <header
        className={cn(
          "max-w-4xl mx-auto text-center pb-6 px-4 relative z-10",
          isDialog ? "pt-10 md:pt-12" : "pt-20"
        )}
      >
        <div className="flex flex-col items-center justify-center">
          ...
          <h1
            className={cn(
              "font-black tracking-tight mt-2 mb-3 text-neutral-900 dark:text-white leading-tight",
              isDialog ? "text-2xl md:text-4xl" : "text-3xl md:text-5xl"
            )}
          >
...
/** Wrapper de la ruta /suscripcion: lee ?status= de la URL. */
function SubscriptionPageFromSearchParams() {
  const searchParams = useSearchParams();
  return <SubscriptionContent variant="page" statusParam={searchParams.get("status")} />;
}
...
export default function SuscripcionesPage() {
  return (
    <Suspense fallback={
      ...
      <SubscriptionPageFromSearchParams />
    </Suspense>
  );
}
